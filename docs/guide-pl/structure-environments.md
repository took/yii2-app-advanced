...
- Każdy programista w grupie ustala swoje własne opcje konfiguracyjne. Zapisywanie takiej konfiguracji może wpływać na
  resztę grupy.
- Hasła baz produkcyjnych i klucze API nie powinny znajdować się w repozytorium.
- Projekt wydawany jest zwykle na wielu środowiskach serwerowych: deweloperskim, testowym, produkcyjnym. Każde z nich
  powinno mieć oddzielną konfigurację.
- Ustalanie wszystkich opcji konfiguracyjnych dla każdego przypadku z osobna jest procesem żmudnym i wysoce powtarzalnym
  i zajmuje zbyt wiele czasu.
